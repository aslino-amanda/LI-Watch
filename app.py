...
aba_diagnostico, aba_base = st.tabs(["🔍 Diagnóstico individual", "📋 Base — lojas para ação"])

with aba_base:
    st.markdown("### Lojas que precisam de ação agora")
    st.caption("Atualizado automaticamente · Lojas ativas dos últimos 60 dias com algum bloqueio")

    if not modo_real:
        st.info("Conecte ao Metabase para ver os dados reais da base.")
    else:
        ...
        if not df_base.empty:
            # Métricas resumo
            total = len(df_base)
            onb   = len(df_base[df_base["status_loja"]=="ONBOARDING INCOMPLETO"])
            nv    = len(df_base[df_base["status_loja"]=="NUNCA VENDEU"])
            svr   = len(df_base[df_base["status_loja"]=="SEM VENDAS RECENTES"])

            m1,m2,m3,m4 = st.columns(4)
            m1.metric("Total para ação", total)
            m2.metric("Onboarding incompleto", onb)
            m3.metric("Nunca vendeu", nv)
            m4.metric("Sem vendas recentes", svr)
            ...
            with col_f1:
                filtro_status = st.selectbox("Status", ["Todos","ONBOARDING INCOMPLETO","NUNCA VENDEU","SEM VENDAS RECENTES"], key="f_status")
            with col_f2:
                filtro_plano = st.selectbox("Plano", ["Todos","PAGO","GRATIS"], key="f_plano")
            with col_f3:
                filtro_seg = st.selectbox("Segmento", ["Todos"] + sorted(df_base["segmento_loja"].dropna().unique().tolist()), key="f_seg")

            df_fil = df_base.copy()
            if filtro_status != "Todos": df_fil = df_fil[df_fil["status_loja"]==filtro_status]
            if filtro_plano  != "Todos": df_fil = df_fil[df_fil["status_plano"]==filtro_plano]
            if filtro_seg    != "Todos": df_fil = df_fil[df_fil["segmento_loja"]==filtro_seg]
            ...
            df_fil = df_fil.sort_values("score", ascending=False)
            ...
            df_fil["prioridade"] = df_fil["score"].apply(_prio)

            # Tabela
            cols_show = ["loja_id","nome_loja","segmento_loja","status_loja",
                        "score","prioridade","dias_cadastro",
                        "qtde_visitas_ultimos_30d","vlr_gmv_ultimos_30d","status_plano"]
            cols_exist = [c for c in cols_show if c in df_fil.columns]
            st.dataframe(
                df_fil[cols_exist].rename(columns={
                    "loja_id":"ID","nome_loja":"Loja","segmento_loja":"Segmento",
                    "status_loja":"Status","score":"Score","prioridade":"Prioridade",
            ...
            st.caption(f"{len(df_fil)} lojas filtradas · Clique em qualquer ID na aba 'Diagnóstico individual' para ver o detalhe")

            # Download
            csv = df_fil[cols_exist].to_csv(index=False).encode("utf-8")
            st.download_button("⬇️ Exportar lista em CSV", data=csv,
